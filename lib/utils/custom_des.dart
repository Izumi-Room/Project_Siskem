...
class _BaseDES {
  ...
  _BaseDES(this.key, this.mode, this.iv) {
    if (iv.length != 8) {
      throw Exception(
          'Invalid Initial Value (iv), must be a multiple of 8 bytes');
    }
  }
}
...
class DES {
  ...
  final List<List<int>> _kN = List.generate(16, (_) => List.filled(48, 0));
  ...
  List<int> _convertToBits(List<int> data) {
    List<int> result = [];
    for (var e in data) {
      String binary = e.toRadixString(2).padLeft(8, '0');
      for (int i = 0; i < 8; i++) {
        result.add(binary.codeUnitAt(i) - 48);
      }
    }
    return result;
  }
  ...
  void _createSubKeys() {
    ...
    List<int> keyBits = _permutate(_pc1, _convertToBits(_baseDES.key));

    // Split into Left and Right sections
    List<int> left = keyBits.sublist(0, 28);
    List<int> right = keyBits.sublist(28, keyBits.length);
    for (int i = 0; i < 16; i++) {
  ...
  List<int> xor(List<int> a, List<int>? b) {
    List<int> result = List.filled(a.length, 0);
  ...
  List<int> crypt(List<int> data, int cryptType) {
    ...
    if (_baseDES.mode == DESMode.CBC) {
      ivBits = _convertToBits(_baseDES.iv);
    }
    ...
    while (i < data.length) {
      ...
      if (_baseDES.mode == DESMode.CBC) {
        if (cryptType == _ENCRYPT) {
          block = xor(block, ivBits);
        }
        processedBlock = _desCrypt(block, cryptType);
        if (cryptType == _DECRYPT) {
          processedBlock = xor(processedBlock, ivBits);
          ivBits = block;
        } else {
          ivBits = processedBlock;
        }
      } else {
        processedBlock = _desCrypt(block, cryptType);
      }

      result.addAll(_convertBitsToIntList(processedBlock));
      i += 8;
    }
...
class DES3 {
  ...
  List<int> encrypt(List<int> data) {
    ...
    if (mode == DESMode.CBC) {
      _desFirst!._baseDES.iv = iv;
      _desSecond._baseDES.iv = iv;
      _desThird!._baseDES.iv = iv;
      int i = 0;
      List<int> result = [];
      while (i < data.length) {
        ...
        _desFirst!._baseDES.iv = block;
        _desSecond._baseDES.iv = block;
        _desThird!._baseDES.iv = block;
        result.addAll(block);
        i += 8;
      }
      return result;
  ...
  List<int> decrypt(List<int> data) {
    if (mode == DESMode.CBC) {
      ...
      for (int i = 0; i < data.length; i += 8) {
        List<int> currentIv = data.sublist(i, i + 8);
        List<int> block = _desThird!.decrypt(currentIv);
        block = _desSecond.encrypt(block);
        block = _desFirst!.decrypt(block);
        _desFirst!._baseDES.iv = currentIv;
        _desSecond._baseDES.iv = currentIv;
        _desThird!._baseDES.iv = currentIv;
        result.addAll(block);
      }
      data = result;
    } else {
...
class DESPadding {
  ...
  static List<int> _pkcs7Unpad(List<int> data) {
    ...
    if (paddingSize < 1 || paddingSize > 8 || paddingSize > data.length) {
      return data;
    }
