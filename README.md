...
### 🎓 Sistem Absensi QR Code dengan Firebase & Enkripsi Triple DES
...
[![Security](https://img.shields.io/badge/Enkripsi-Triple%20DES-B91C1C?style=for-the-badge)](#-catatan-keamanan)

<br/>

> 🛡️ *Data kehadiran tidak hanya dicatat — setiap byte-nya melewati lapisan enkripsi untuk kebutuhan pembuktian keamanan data.*
...
**Absensi Triple DES** adalah aplikasi absensi berbasis Flutter untuk kebutuhan demo skripsi, tugas akhir, atau pembelajaran kriptografi terapan. Aplikasi ini menggabungkan:

- 📷 **QR Code** sebagai media sesi absensi
- ☁️ **Firebase** sebagai backend cloud
- 🔒 **Triple DES** sebagai mekanisme enkripsi payload

Alur utamanya sederhana: admin membuat sesi absensi → aplikasi menghasilkan QR Code terenkripsi → mahasiswa memindai kode tersebut → data kehadiran disimpan ke Firebase Realtime Database.

> ⚠️ **Catatan:** Project ini bersifat edukatif. Untuk production, aturan database, manajemen key, validasi sesi, dan proteksi replay attack perlu diperkuat.
...
| 🗄️ Database | Data pengguna, sesi, dan absensi tersimpan di Firebase Realtime Database |
...
🔐 Login                               🔐 Login
    │                                      │
    ▼                                      ▼
📝 Buat sesi absensi               📷 Scan QR Code
    │                                      │
    ▼                                      ▼
📦 Payload:                         🔓 Dekripsi payload sesi
   admin_id│tanggal│jam│                   │
   status│random_key                       ▼
    │                               📦 Payload:
    ▼                                  user_id│session_id│tanggal│
🔒 Enkripsi Triple DES                 jam│status│random_key
    │                                      │
    ▼                                      ▼
📲 QR Code ditampilkan             🔒 Enkripsi Triple DES
    +                                      │
💾 Sesi disimpan ke Firebase              ▼
...
- ⌨️ Input ciphertext manual untuk kebutuhan demo
...
## 🗄️ Skema Firebase Realtime Database
...
> 📍 Default URL database berada di: `lib/services/realtime_database_service.dart`
...
**🛡️ Build release dengan obfuscation:**
```bash
...
> ⚠️ Pastikan hal-hal berikut sebelum deployment!
